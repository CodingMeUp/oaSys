...
class BugAssigned extends Component {
  constructor(props) {
    super(props);
  }
  componentDidMount() {
  ...
  onOk() {
    const comment = window['win_editor_assigned'].html();
    const _this = this;
  
    Ajax.post({
      url: API.BUG_UPDATE_ASSIGNEDTO,
      data: {
        comment: comment,
        ...
        mailto : this.props.form.getFieldValue('mailto')
      },
      success(res) {
  ...
  render() {
    ...
    if (this.loadUi) {
      ui = (<Form horizontal form={this.props.form}>
          ...
          label="备注"
          >
          <Editor editorId="win_editor_assigned" editorName="win_editor_assigned" />
        </FormItem>
      </Form>);
    }
    ...
    return (
      <div>
        <Modal title={<span><FAIcon type="fa-hand-o-right" /> {bug.id} {bug.title} → 指派给 </span>}
...
BugAssigned = Form.create()(BugAssigned);
