...
const SelfTestResultList = React.createClass({
  getInitialState() {
    return {
      loading: false,
      selectModuleName: '',
  ...
  componentDidMount() {
    ...
    this.projectId = this.props.location.query.projectId;
  ...
  showDoResultDetail(o, doResult, row, e) {
    if (+o > 0) {
      Modal.info({
        ...
        width: '96%',
        content: (
          <SelfTestAllResult doResult={doResult} projectId={row.projectId} version={row.version} env={row.env} times={row.times} userId={row.caseDoUser} />
        ),
        onOk() { },
      });
    }
  ...
  renderTGS(o, row, index) {
    return (
      <Tooltip placement='bottom' title='查看明细'>
        <span><a onClick={this.showDoResultDetail.bind(this, o, '通过', row) }>{o}</a></span>
      </Tooltip>
    );
  },
  renderBTGS(o, row, index) {
    return (
      <Tooltip placement='bottom' title='查看明细'>
        <span><a onClick={this.showDoResultDetail.bind(this, o, '不通过', row) }>{o}</a></span>
      </Tooltip>
    );
  },
  renderZSS(o, row, index) {
    return (
      <Tooltip placement='bottom' title='查看明细'>
        <span><a onClick={this.showDoResultDetail.bind(this, o, '阻塞', row) }>{o}</a></span>
      </Tooltip>
    );
  },
  renderYLS(o, row, index) {
    return (
      <Tooltip placement='bottom' title='查看明细'>
        <span><a onClick={this.showDoResultDetail.bind(this, o, null, row) }>{o}</a></span>
      </Tooltip>
    );
  },
  render() {
...
export default SelfTestResultList;
